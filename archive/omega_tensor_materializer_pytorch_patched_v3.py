"""
THE OMEGA PROTOCOL: THE TENSOR MATERIALIZER (ROCm GPU Backend V3)
...
def materialize_shards(base_l1_dir: str, output_dir: str, target_years: list = None):
    ...
    for file in parquet_files:
        ...
        try:
            ...
            # Use completely decoupled batch reading to prevent unified memory page faults
            # We don't wrap the pandas/pyarrow logic in torch.no_grad() or any torch context
            # We only use torch explicitly when pushing arrays to device
            for batch in parquet_file.iter_batches(batch_size=5_000_000, columns=["symbol", "time", "price", "vol_tick"]):
                ...
                for sym, group_df in grouped:
                    ...
                    arr_time = group_df["time"].to_numpy()
                    ...
                    # EXPLICIT synchronization point before sending to GPU.
                    # Prevent pinned memory racing with pyarrow.
                    if device.type == 'cuda':
                    ...
                    with torch.no_grad():
                        ...
                        for i in range(0, original_len, CHUNK_MAX):
                            ...
                            # EXPLICIT pull to CPU before continuing to next chunk
                            chunk_cpu = feature_tensor_chunk.cpu().numpy()
                            all_feature_chunks.append(chunk_cpu)
                            
                            del pc_chunk, of_chunk, mv_chunk, vol_chunk, feature_tensor_chunk
                            # Keep VRAM as clean as possible inside the tight loop
                            if device.type == 'cuda':
