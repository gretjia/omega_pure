"""
THE OMEGA PROTOCOL: THE KOLMOGOROV COMPRESSOR (BLITZKRIEG V2 - AUDITED)
Paradigm: Successive Halving & Fast Exploration
Discipline: FLOAT64 PRECISION. TENSOR PRE-ALLOCATION. SHARD ISOLATION.
"""
import sys
import argparse
import gc
...
# =====================================================================
# MODULE 2: AUDITED DATA INGESTION (PRE-ALLOCATION + FLOAT64 STATS)
# =====================================================================
def load_and_split_data_audited(gcs_path, limit_files=20): 
    print(f"[BLITZ-V2] Audited Ingestion: {limit_files} shards...", flush=True)
    fs = gcsfs.GCSFileSystem()
    files = sorted(fs.glob(f"{gcs_path}/*.parquet"))
    if not files: raise ValueError(f"FATAL: No shards at {gcs_path}")
    files = files[:limit_files]
    
    # 1. 预扫描计算总行数与训练集切分 (防止 cat 操作内存翻倍)
    print(f"[BLITZ-V2] Pre-scanning shards for memory pre-allocation...", flush=True)
    shard_info = []
    total_rows = 0
    ...
    for f in files:
        ...
        with fs.open(f) as pf:
            ...
            table_meta = pq.read_metadata(pf)
            rows = table_meta.num_rows
            shard_info.append((f, rows))
            total_rows += rows
            
    print(f"[BLITZ-V2] Total rows identified: {total_rows:,}. Allocating RAM...", flush=True)
    
    # 2. 预分配大张量 (32GB RAM 容纳 1.4B+ 行 float32 没问题，只要不副本)
    full_data = torch.empty((total_rows, 3), dtype=torch.float32)
    
    # 3. 隔离计算训练集统计量 (使用 Float64 保证精确归一化)
    # 按照 80/20 切分碎片
    split_idx_shard = int(len(files) * 0.8)
    train_shards = shard_info[:split_idx_shard]
    
    print(f"[BLITZ-V2] Phase 1: Calculating Train-set stats (Float64 Precision)...", flush=True)
    ...
    print(f"[BLITZ-V2] Stats Computed. Mean: {mean}, Std: {std}", flush=True)
    
    # 4. 逐个读入，在位归一化并填入预分配张量
    print(f"[BLITZ-V2] Phase 2: Loading & Normalizing shards into pre-allocated memory...", flush=True)
    current_pos = 0
    ...
    for f, rows in shard_info:
        ...
        shard_tensor.sub_(mean_t).div_(std_t) # In-place normalization
        full_data[current_pos : current_pos + rows] = shard_tensor
        current_pos += rows
        del df, shard_tensor; gc.collect()
        
    split_point = train_rows
    train_data, val_data = full_data[:split_point], full_data[split_point:]
...
def forge_compressor(args):
    ...
    print(f"[BLITZ-V2] Node Awake | Seq: {args.seq_len} | Stride: {args.stride}", flush=True)
    
    # 采用审计后的加载方案
    train_data, val_data = load_and_split_data_audited(args.gcs_input, limit_files=20)
    ...
    for logical_step in range(1, args.max_steps + 1):
        ...
        for _ in range(accum_steps):
            ...
            if torch.isnan(fvu_loss) or torch.isinf(fvu_loss):
                print(f"🚨 [FATAL] NaN detected at step {logical_step}. Suiciding.", flush=True)
