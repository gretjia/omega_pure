"""
THE OMEGA PROTOCOL: PURE MATH ANONYMOUS PROOF (MEMORY SAFE)
Paradigm: Epiplexity Validation on Faceless Tensors
Discipline: NO ML. PURE PHYSICS. A-SHARE LIMIT FILTERS. OOM IMMUNITY.
"""
import os
import numpy as np
import pandas as pd
...
def verify_math_core_on_shard(file_path, z_score_trigger=-3.0, hold_ticks=2400):
    try:
        # Load only 10% of the shard to prevent OOM
        # Pandas backend of pyarrow can filter columns but reading full file still uses lots of memory if files are huge
        df = pd.read_parquet(file_path, columns=['price', 'order_flow', 'srl_residual', 'epiplexity'])
        # Downsample drastically to ensure memory safety on 128GB node even with 16 parallel workers
        df = df.iloc[::2] 
...
def run_anonymous_proof(data_dir):
    files = sorted(glob(f"{data_dir}/*.parquet"))[:30] # 极度降低抽样文件数到30，绝对防爆
    print(f"🚀 [OMEGA PROOF] Igniting Math Core Verification on {len(files)} Anonymous Shards...")

    all_pnls = []
    # 限制并发核心数，防止 64 个进程同时 read_parquet 打穿内存
    max_workers = min(8, os.cpu_count() or 4) 
    print(f"⚙️ Using {max_workers} memory-safe workers...")
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        for i, future in enumerate(as_completed([executor.submit(verify_math_core_on_shard, f) for f in files])):
            ...
            if (i+1) % 5 == 0: print(f"   Scanned {i+1}/{len(files)} shards...")
